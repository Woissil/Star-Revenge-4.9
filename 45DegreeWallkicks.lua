...
if network_is_server() or network_is_moderator() then
    hook_chat_command('wallkickdegrees', "Sets the angle limit for bonking. Type in degrees.",
    function (msg)
        if tonumber(msg) ~= nil then
            msg = tonumber(msg)
            if (msg < 0) then
                djui_chat_message_create("No negative numbers.")
                return true
            elseif (msg > 180) then
                djui_chat_message_create("This number is too high.")
                return true
            else
                djui_chat_message_create("Wallkick degrees set.")
                gGlobalSyncTable.limit = msg + 1
                return true
            end
        else
